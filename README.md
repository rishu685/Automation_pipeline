...
## 📹 Screen Recording Script & Walkthrough
*Use this exact timeline to record your unedited screen share. Keep Chrome open on the right and VS Code/Notion on the left:*

* **0:00 - 0:45 (The Setup):**
  * **Visual:** Split screen showing your Notion database on one side and your local Streamlit app (`http://localhost:8501`) on the other. Open the sidebar in Streamlit to show the green checks.
  * **Audio:** *"Hey everyone, today we're automating client onboarding. Right now, when a new client signs up, we have to manually copy their info into Slack and write a custom welcome email. We're going to automate this using Notion, Slack, Resend, and Gemini."*
* **0:45 - 1:15 (The Trigger):**
  * **Visual:** Click on Notion. Add a new row:
    * *Name:* `Sarah Jenkins`
    * *Email:* [Your own email address]
    * *Company:* `Leafygreen Coffee`
    * *Notes:* `Wants to build a Shopify subscription box funnel for coffee beans.`
    * *Status:* Set to `Not started`.
  * **Audio:** *"I've just added a new client, Sarah Jenkins from Leafygreen Coffee. Notice her status is set to 'Not started'."*
* **1:15 - 1:50 (The Automation):**
  * **Visual:** Switch to the Streamlit tab. Click the red **`Scan & Onboard Clients`** button. Show the execution logs expanding.
  * **Audio:** *"Now, I'll trigger the scan from our dashboard. Our script is fetching the row from Notion, sending the client notes to Gemini to draft the assets, posting the notification to Slack, delivering the welcome email, and updating the database."*
* **1:50 - 2:30 (The Results):**
  * **Visual:** 
    * Switch to Slack: show the message in `#new-channel`.
    * Switch to Email: show the HTML email in your inbox.
    * Switch to Notion: show the status column has updated to `Done`.
  * **Audio:** *"And it's complete! Here is the Slack message generated by Gemini summarizing Sarah's goals and listing three onboarding action items. Here is the custom HTML welcome email sent directly to her inbox. And finally, if we look back at Notion, her status has automatically flipped to 'Done'. Zero admin overhead, all handled by AI."*
